...
class MovieLensTripletDataset(Dataset):
    ...
    def generate_triplets(self, df):
        ...
        for user, user_data in grouped:
            pos_movies = user_data[user_data["rating"] >= POSITIVE_THRESHOLD][
                "movieId"
            ].values
            neg_movies = user_data[user_data["rating"] < POSITIVE_THRESHOLD][
                "movieId"
            ].values
            ...
            for pos_movie in pos_movies:
                neg_movie = np.random.choice(
                    neg_movies
                )  # Randomly sample a negative movie
